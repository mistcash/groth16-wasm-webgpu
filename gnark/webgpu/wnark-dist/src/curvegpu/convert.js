export function splitBigUint64WordsToU32(words) {
    ...
    for (let i = 0; i < words.length; i += 1) {
        const word = words[i];
        out[2 * i] = Number(word & 0xffffffffn);
        out[2 * i + 1] = Number((word >> 32n) & 0xffffffffn);
    }
    return out;
}
export function splitBytesLEToU32(bytes) {
...
export function joinU32LimbsToBigUint64(limbs) {
...
export function joinU32LimbsToBytesLE(limbs) {
    if (limbs.length !== 8 && limbs.length !== 12) {
        throw new Error(`unsupported gpu limb count ${limbs.length}`);
    }
