export function mustElement(value, name) {
    if (value === null) {
        throw new Error(`missing element: ${name}`);
    }
    return value;
}
export function createPageUI(statusEl, logEl) {
    return {
        setStatus(text) {
...
export async function getAdapterInfo(adapter) {
...
export async function appendAdapterDiagnostics(adapter, lines) {
    const adapterWithFallback = adapter;
