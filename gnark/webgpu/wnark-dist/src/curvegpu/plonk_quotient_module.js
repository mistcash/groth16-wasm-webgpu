import { createSimpleStorageBuffer, createSimpleStorageBufferFromBytes, createSimpleUniformBuffer, loadShaderParts, readbackSimpleBuffer, } from "./runtime_common.js";
...
const PLONK_QUOTIENT_SHADER_PARTS = {
    bn254: [
...
function repeatEachPackedVector(values, vectorBytes, repeatCount) {
...
function unpackU32LE(bytes, count, label) {
...
export function createPlonkQuotientModule(config) {
    ...
    const quotientKernels = new Map();
    let dynamicTransformCache = null;
    const staticMontCache = new Map();
    const auxMontCache = new Map();
    async function getQuotientKernel(commitmentCount) {
    ...
    async function runQuotientKernelMont(vectorsMontPacked, blindsMontPacked, scalarsMontPacked, elementCount, blindCoeffCount, commitmentCount, cosetCount = 1) {
        ...
        const paramsBuffer = createSimpleUniformBuffer(device, "plonk-quotient-params", new Uint32Array([elementCount, blindCoeffCount, cosetCount, 0]));
        try {
    ...
    async function transformAndEvaluateQuotientCoset(input) {
        const { dynamicValuesPacked, scalingPacked, staticValuesPacked, twiddlesPacked, denominatorsPacked, blindsPacked, scalarsPacked, elementCount, blindCoeffCount, commitmentCount, dynamicTransformCacheKey = 0, staticMontCacheKey = 0, } = input;
        const elementBytes = fr.byteSize;
        const vectorBytes = elementCount * elementBytes;
        ...
        const canReuseDynamicCache = dynamicTransformCacheKey > 0 &&
        ...
        if (dynamicValuesPacked.byteLength !== expectedDynamicBytes && !(dynamicValuesPacked.byteLength === 0 && canReuseDynamicCache)) {
            throw new Error(`PLONK quotient transform/evaluate expected ${expectedDynamicBytes} dynamic bytes, got ${dynamicValuesPacked.byteLength}`);
        }
        ...
        const canReuseStaticCache = staticMontCacheKey > 0 &&
        ...
        if (staticValuesPacked.byteLength !== expectedStaticBytes && !(staticValuesPacked.byteLength === 0 && canReuseStaticCache)) {
            throw new Error(`PLONK quotient transform/evaluate expected ${expectedStaticBytes} static bytes, got ${staticValuesPacked.byteLength}`);
        }
        ...
        const dynamicCoeffMont = cachedDynamicCoeffMont
    ...
    async function transformAndEvaluateQuotientCosets(input) {
        const { dynamicValuesPacked, scalingPacked, staticValuesPacked, staticMontCacheKeysPacked, twiddlesPacked, denominatorsPacked, blindsPacked, scalarsPacked, elementCount, blindCoeffCount, commitmentCount, dynamicTransformCacheKey = 0, cosetCount, auxMontCacheKey = 0, } = input;
        const elementBytes = fr.byteSize;
        const vectorBytes = elementCount * elementBytes;
        if (!Number.isInteger(elementCount) || elementCount <= 0 || (elementCount & (elementCount - 1)) !== 0) {
            throw new Error(`invalid PLONK quotient evaluate element count ${elementCount}`);
        ...
        if (dynamicValuesPacked.byteLength !== expectedDynamicBytes) {
            throw new Error(`PLONK quotient cosets expected ${expectedDynamicBytes} dynamic bytes, got ${dynamicValuesPacked.byteLength}`);
        }
        const cachedAux = auxMontCacheKey > 0 ? auxMontCache.get(auxMontCacheKey) : undefined;
        const canReuseAuxCache = scalingPacked.byteLength === 0 &&
        ...
        const canReuseStaticCache = staticValuesPacked.byteLength === 0 &&
        ...
        if (staticValuesPacked.byteLength !== expectedStaticBytes && !canReuseStaticCache) {
            throw new Error(`PLONK quotient cosets expected ${expectedStaticBytes} static bytes, got ${staticValuesPacked.byteLength}`);
        }
        ...
        if (denominatorsPacked.byteLength !== cosetCount * vectorBytes && !canReuseAuxCache) {
            throw new Error(`PLONK quotient cosets expected ${cosetCount * vectorBytes} denominator bytes, got ${denominatorsPacked.byteLength}`);
        }
        const blindBytes = PLONK_QUOTIENT_BLIND_COUNT * blindCoeffCount * elementBytes;
        ...
        const shiftedCoeffMont = await fr.mulPackedMont(repeatPackedVector(dynamicCoeffMont, cosetCount), repeatEachPackedVector(scalingMont, vectorBytes, dynamicVectorCount));
        ...
        for (let i = 0; i < cosetCount; i += 1) {
            ...
            vectorsMontPacked.set(staticMont.subarray(staticStart, staticStart + staticVectorCount * vectorBytes), vectorsStart + dynamicVectorCount * vectorBytes);
            ...
            vectorsMontPacked.set(denominatorsMont.subarray(denominatorStart, denominatorStart + vectorBytes), vectorsStart + (dynamicVectorCount + staticVectorCount + 1) * vectorBytes);
        }
    ...
    async function preloadQuotientStaticAndAux(input) {
        const { staticValuesPacked, staticMontCacheKeysPacked, scalingPacked, twiddlesPacked, denominatorsPacked, elementCount, staticVectorCount, cosetCount, auxMontCacheKey, } = input;
        const elementBytes = fr.byteSize;
        const vectorBytes = elementCount * elementBytes;
        if (!Number.isInteger(elementCount) || elementCount <= 0 || (elementCount & (elementCount - 1)) !== 0) {
        ...
        if (denominatorsPacked.byteLength !== cosetCount * vectorBytes) {
            throw new Error(`PLONK quotient preload expected ${cosetCount * vectorBytes} denominator bytes, got ${denominatorsPacked.byteLength}`);
        }
    ...
    return {
        ...
        async prewarmPlonkQuotientEvaluateKernel(commitmentCount = 0) {
