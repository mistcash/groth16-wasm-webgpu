...
import { createBindGroupForBuffers, createEmptyPointStorageBuffer, createParamsBuffer, createStorageBufferFromBytes, createU32StorageBuffer, readbackBuffer, submitKernel, } from "./msm_gpu_runtime.js";
export function buildJacPippengerRuntime(kernels, workgroupSize = 64, debug = false) {
    return {
        bucket: kernels.bucket,
        bucketWorkgroupSize: workgroupSize,
        combine: kernels.combine,
        async reduceWindows(options) {
            const { device, pool, pointBytes, uniformBytes, zeroInput, bucketOutput, bucketCountOut, bucketValuesInput, windowStartsInput, windowCountsInput, metadata, count, labelPrefix, } = options;
            ...
            const weightBindGroup = createBindGroupForBuffers(device, kernels.weightJac, `${labelPrefix}-weight-bg`, bucketOutput, zeroInput, weightedBucketOutput, weightParams, bucketValuesInput);
            ...
            const windowBindGroup = createBindGroupForBuffers(device, kernels.subsumJac, `${labelPrefix}-window-bg`, weightedBucketOutput, zeroInput, windowOutput, windowParams, bucketValuesInput, windowStartsInput, windowCountsInput);
            await submitKernel(device, kernels.subsumJac, windowBindGroup, count * metadata.numWindows * workgroupSize, `${labelPrefix}-window`, workgroupSize, debug);
...
export async function runSparseSignedPippengerMSM(options) {
    const { device, pool, runtime, basesBytes, pointBytes, uniformBytes, zeroPointBytes, scalarWords, count, termsPerInstance, window, maxChunkSize = 256, labelPrefix, debug = false, } = options;
    ...
    const bucketBindGroup = createBindGroupForBuffers(device, runtime.bucket, `${labelPrefix}-bucket-bg`, basesInput, zeroInput, bucketOutput, bucketParams, baseIndicesInput, bucketPointersInput, bucketSizesInput);
    ...
    const finalBindGroup = createBindGroupForBuffers(device, runtime.combine, `${labelPrefix}-final-bg`, windowOutput, zeroInput, finalOutput, finalParams);
