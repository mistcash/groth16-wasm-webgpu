...
function collectRequestedLimits(adapter, options) {
...
function buildDiagnostics(adapter, adapterInfo) {
    return {
 ...
 */
export async function createCurveGPUContext(options = {}) {
    if (!navigator.gpu) {
        throw new CurveGPUNotSupportedError("WebGPU is not supported in this browser. " +
    ...
    if (!adapter) {
        throw new CurveGPUNotSupportedError("requestAdapter returned null. " +
    ...
    const maxWorkgroupSize = device.limits.maxComputeWorkgroupSizeX ?? 256;
