...
export function lazyAsync(factory) {
    let promise = null;
    return () => {
        if (!promise) {
            promise = factory();
...
export function ensureByteLength(bytes, expected, label) {
    if (bytes.byteLength !== expected) {
        throw new Error(`${label}: expected ${expected} bytes, got ${bytes.byteLength}`);
    }
}
export function packElementBatch(values, elementBytes, label) {
...
export function unpackElementBatch(bytes, elementBytes, count) {
    const out = [];
...
export async function loadShaderText(path) {
    return fetchText(path);
}
export async function loadShaderParts(parts) {
    return fetchShaderParts(parts);
}
export function createSimpleStorageBuffer(device, label, size, usage = GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST) {
...
export function createSimpleStorageBufferFromBytes(device, label, bytes, usage = GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST) {
...
export function createSimpleUniformBuffer(device, label, uniformWords) {
    const buffer = createSimpleStorageBuffer(device, label, uniformWords.byteLength, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST);
    device.queue.writeBuffer(buffer, 0, uniformWords.buffer, uniformWords.byteOffset, uniformWords.byteLength);
    return buffer;
}
export function createSimpleBindGroup(device, kernel, label, inputA, inputB, output, uniform) {
...
export async function submitSimpleKernel(device, kernel, bindGroup, workgroups, label) {
...
export async function readbackSimpleBuffer(device, buffer, outputBytes, label) {
    let mapped = false;
    const readbackBuffer = createSimpleStorageBuffer(device, `${label}-readback`, outputBytes, GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ);
    try {
