...
function packAffinesToJacobianPacked(bases, componentBytes, pointBytes, montOne) {
    const out = new Uint8Array(bases.length * pointBytes);
    for (let i = 0; i < bases.length; i += 1) {
        const base = i * pointBytes;
        const b = bases[i];
        const isInfinity = b.x.c0.every((byte) => byte === 0) &&
...
export function createG2MSMModule(context, options, g2, fp) {
    ...
    async function runBatch(bases, scalars, msmOptions = {}) {
        ...
        const scalarWords = packScalarWords(scalars);
    ...
    return {
        ...
        async pippengerAffineBatch(bases, scalars, msmOptions) {
            return runBatch(bases, scalars, msmOptions);
        },
        async pippengerPackedJacobianBases(basesPacked, scalarsPacked, msmOptions) {
            const count = msmOptions.count ?? 1;
